...
class Admin(commands.Cog):
    ...
    async def selfroleset_add(self, ctx: commands.Context, *, role: discord.Role):
        ...
        if not self.pass_user_hierarchy_check(ctx, role):
            await ctx.send(
                _(
                    "I cannot let you add {role.name} as a selfrole because that role is higher than or "
                    "equal to your highest role in the Discord hierarchy."
                ).format(role=role)
            )
            return
    ...
    async def selfroleset_remove(self, ctx: commands.Context, *, role: SelfRole):
        ...
        if not self.pass_user_hierarchy_check(ctx, role):
            await ctx.send(
                _(
                    "I cannot let you remove {role.name} from being a selfrole because that role is "
